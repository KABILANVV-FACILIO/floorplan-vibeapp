...
export function reducer(state: AppState, action: Action): AppState {
  switch (action.type) {
    ...
    case 'SET_MODULE_ENABLED': {
      ...
      // Turning a module off has to evict it from every transient pointer too, or the app is left
      // holding a selection, filter or armed tool for something that no longer renders.
      const hidden = (u: Unit | undefined | null) => !!u && u.type === action.module;
      // A disabled module's plan type disappears from the switcher, so a planId pointing at it
      // would strand the canvas on a plan with no way back.
      const planId: PlanId = state.planId === action.module ? 'workstation' : state.planId;
      const nextPlanId: PlanId =
        planId === 'workstation' && !enabledModules.workstation
          ? ((['locker', 'parking'] as const).find((p) => enabledModules[p]) ?? 'custom')
          : planId;
      return {
        ...state,
        enabledModules,
        planId: nextPlanId,
        tool: state.tool === action.module ? 'select' : state.tool,
        spaceFilter: state.spaceFilter === action.module ? 'all' : state.spaceFilter,
        settingsTab: state.settingsTab === action.module ? 'permissions' : state.settingsTab,
        selected: hidden(state.units.find((u) => u.id === state.selected)) ? null : state.selected,
        multiSelected: state.multiSelected.filter((id) => !hidden(state.units.find((u) => u.id === id))),
        mobSel: hidden(state.units.find((u) => u.id === state.mobSel)) ? null : state.mobSel,
        bookForm: hidden(state.units.find((u) => u.id === state.bookForm?.unitId)) ? null : state.bookForm,
      };
    }
