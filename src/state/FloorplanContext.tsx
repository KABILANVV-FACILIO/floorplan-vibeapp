...
function buildActions(state: AppState, dispatch: Dispatch<Action>, canvasRectRef: MutableRefObject<DOMRect | null>) {
  ...
  return {
    ...
    closeDraft: async () => {
      ...
      const label = nextLabel(state, zoneType, TYPE_META[zoneType].prefix);
      const base: Unit = {
        id: 'u' + Date.now(),
        type: zoneType,
        label,
    ...
    applyAutoMap: (mapping: Record<string, UnitType | 'ignore'>) => {
      ...
      const countOf = (t: UnitType) => state.units.filter((u) => u.type === t).length;
      const counters: Record<UnitType, number> = {
        workstation: countOf('workstation'),
        locker: countOf('locker'),
        parking: countOf('parking'),
        room: countOf('room'),
        delivery: countOf('delivery'),
        amenity: countOf('amenity'),
      };
    ...
    assign: async (contactId: string, unitId: string) => {
      ...
      const contactName = MOCK_EMPLOYEES.find((c) => c.id === contactId)?.name ?? contactId;
