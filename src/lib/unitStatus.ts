...
export function markerStyle(state: AppState, unit: Unit, markerScale = 1): MarkerStyle {
  ...
  const avail = moduleColor(state, unit.type, isRoomLike(unit.type) ? 'available' : 'free');
...
export function unitStatus(state: AppState, unit: Unit, contactName: (id: string) => string): UnitStatus {
  ...
  return { key: 'available', text: 'Available', bg: TOKEN.success050, fg: TOKEN.success700, dot: moduleColor(state, unit.type, isRoomLike(unit.type) ? 'available' : 'free') };
}
