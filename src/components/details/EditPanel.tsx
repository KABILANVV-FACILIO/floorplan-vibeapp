...
function ToolsTab() {
  ...
  if (placingUnit) {
    ...
    if (isZoneTool(state.tool)) bannerHint = 'Click corners on the plan; click the first point or press Enter to finish.';
  ...
  // Zone tools appear only while their module is on, so a disabled module offers no way to draw
  // something the rest of the app would then refuse to show.
  const workTools: { id: EditTool; label: string; title: string }[] = [
    { id: 'select', label: 'Select', title: 'Select & move (V)' },
    ...(moduleEnabled(state, 'room') ? [{ id: 'room' as EditTool, label: 'Room', title: 'Draw a room' }] : []),
    ...(moduleEnabled(state, 'delivery') ? [{ id: 'delivery' as EditTool, label: 'Delivery', title: 'Draw a delivery area' }] : []),
    ...
    <div className={card.card}>
      <div className={card.cardBody}>
        ...
        <div className={styles.addGrid}>
          {ADD_TOOLS.filter((t) => moduleEnabled(state, t.id)).map((t) => {
  ...
  return (
    <div className={card.card}>
      <div className={card.cardBody}>
        ...
        {!isRoomLike(sel.type) && sel.type !== 'amenity' && (
