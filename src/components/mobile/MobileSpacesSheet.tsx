...
export function MobileSpacesSheet({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  // Disabled modules never reach the list, its counts or its filter chips.
  const shown = useMemo(() => state.units.filter((u) => moduleEnabled(state, u.type)), [state.units, state.enabledModules]);

  const counts = useMemo(() => {
    const c: Record<string, number> = { all: shown.length };
    for (const u of shown) c[u.type] = (c[u.type] || 0) + 1;
    return c;
  }, [shown]);
