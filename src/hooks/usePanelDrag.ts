...
export function usePanelDrag(id: 'context' | 'portfolio' | 'details', width: number) {
  ...
  // Clears the stage's bottom overlays when there's room, and never exceeds the stage itself
  // when there isn't — see panelMaxHeight.
  const maxH = panelMaxHeight(state.stage.h, pos.y);
